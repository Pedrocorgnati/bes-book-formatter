...
impl SidecarManager {
    ...
    /// Spawn pandoc with given arguments (module-4 DOCX generation).
    pub async fn spawn_pandoc(args: &[String], timeout_ms: u64) -> Result<(String, String), AppError> {
        Self::spawn_process("pandoc", args, timeout_ms).await
    }
    ...
    async fn spawn_process(
        ...
        match tokio::time::timeout(timeout, child.wait_with_output()).await {
            Ok(Ok(output)) => {
                ...
                // SEC-008: Truncate stderr to 100 chars to avoid leaking manuscript content in logs
                let stderr_truncated = if stderr_full.len() > 100 {
                    format!("{}…[truncated]", &stderr_full[..100])
                } else {
                    stderr_full.clone()
                };

                if output.status.success() {
                    Ok((stdout, stderr_truncated))
                } else {
