<script lang="ts">
  import { locale, setLocale, availableLocales, t, type Locale } from '$lib/i18n/engine';
...
<div data-testid="header-language-selector" class="lang-selector" aria-label={t('settings.language')}>
  <label for="lang-select" class="sr-only">{t('settings.language')}</label>
  <select
