...
<style>
  ...
  .epb__sync-btn:hover:not(:disabled) { background: var(--color-surface-hover); }
  ...
  .phase--pending  { border-color: var(--color-border); }
  .phase--blocked  { border-color: var(--color-danger, #ef4444);  background: color-mix(in srgb, var(--color-danger, #ef4444) 10%, transparent); }
  .phase--skipped  { border-color: var(--color-border); opacity: 0.5; }

  .phase__id   { font-weight: 700; font-size: 0.6875rem; letter-spacing: 0.025em; color: var(--color-text-muted); }
  .phase__icon { font-size: 1rem; }
  .phase__name { font-weight: 500; line-height: 1.2; }
  .phase__date { font-size: 0.625rem; color: var(--color-text-muted); }
