<script lang="ts">
  ...
  function scheduleSave(partial: Record<string, unknown>) {
    ...
    saveTimer = setTimeout(() => persistConfig(partial), 500);
  }
  ...
  async function persistConfig(partial: Record<string, unknown>) {
    ...
    try {
      ...
      console.error('[TypographyPanel] save error:', err);
    } finally {
  ...
  function onOrphanControlChange(e: Event) {
    ...
    if (!isNaN(val) && val >= 1 && val <= 6) scheduleSave({ orphanControl: val });
  }

  function onWidowControlChange(e: Event) {
    const val = parseInt((e.target as HTMLInputElement).value);
    if (!isNaN(val) && val >= 1 && val <= 6) scheduleSave({ widowControl: val });
  }
