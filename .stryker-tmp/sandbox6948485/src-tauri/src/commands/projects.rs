...
/// Update a project's metadata (name, genre, language, scores).
///
/// Frontend: `const project = await invoke<ApiResponse<Project>>('update_project', { id, data: { name: 'New Name' } });`
#[tauri::command]
pub async fn update_project(
    pool: State<'_, SqlitePool>,
    id: String,
    data: crate::models::UpdateProject,
) -> Result<ApiResponse<Project>, String> {
    let repo = ProjectRepository::new(pool.inner().clone());

    // Verify project exists
    match repo.find_by_id(&id).await {
        Ok(None) => return Ok(ApiResponse::err(format!("PROJECT_080: Project not found: {}", id))),
        Err(e) => return Ok(ApiResponse::err(e.message)),
        _ => {}
    }

    match repo.update(&id, &data).await {
        Ok(project) => Ok(ApiResponse::ok(project)),
        Err(e) => Ok(ApiResponse::err(e.message)),
    }
}
