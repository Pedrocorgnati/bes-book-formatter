<script lang="ts">
  ...
  import { generationStore } from '$lib/stores/generationStore';
  ...
  async function runPreflight() {
    ...
    generationStore.startPreflight(currentSelection.formats[0] ?? 'epub3', currentSelection.platform);
    try {
      ...
      toastStore.error(`Preflight falhou: ${e}`);
    } finally {
  ...
  async function startGeneration() {
    ...
    for (const format of formats) {
      try {
        ...
        anyError = true;
        toastStore.error(`Erro ao gerar ${format}: ${e}`);
      }
    }
    ...
    if (lastResult) {
      ...
      generationStore.setError('Um ou mais formatos falharam');
    }
...
<div data-testid="generation-panel" class="gen-panel">
  ...
  <div class="gen-panel__header">
    ...
    {#if genState.status === 'idle' || genState.status === 'done' || genState.status === 'error'}
