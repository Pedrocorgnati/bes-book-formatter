<script lang="ts">
  ...
  import { ipcListIllustrations, ipcUpdateIllustrationAltText } from '$lib/ipc/typography';
  ...
  type Filter = 'all' | 'pending' | 'imported' | 'linked' | 'error';
  ...
  const counts = $derived({
    ...
    pending:  illustrations.filter((i) => i.state === 'pending').length,
    imported: illustrations.filter((i) => i.state === 'imported').length,
    linked:   illustrations.filter((i) => i.state === 'linked').length,
    error:    illustrations.filter((i) => i.state === 'error').length,
  });
  ...
  const filters: { key: Filter; label: string }[] = [
    { key: 'all',      label: t('illustrations.filterAll') },
    { key: 'pending',  label: t('illustrations.filterPending') },
    { key: 'imported', label: t('illustrations.filterImported') },
    { key: 'linked',   label: t('illustrations.filterLinked') },
    { key: 'error',    label: t('illustrations.filterError') },
  ];
  ...
  async function loadIllustrations() {
    ...
    try {
      ...
      console.error('[IllustrationGallery] load error:', err);
    } finally {
  ...
  async function saveAltText() {
    ...
    try {
      ...
      console.error('[IllustrationGallery] alt-text save error:', err);
    } finally {
      altTextSaving = false;
    }
  }
  ...
  const altTextSaveEnabled = $derived(altTextValue.trim().length >= 10);
</script>
...
<style>
  ...
  .gallery-empty p { font-size: 0.875rem; margin: 0; }
