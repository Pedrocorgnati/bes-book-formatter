...
/// Generate a Table of Contents from the parsed ManuscriptAST.
/// Returns a structured TOC with chapter titles, page numbers (estimated), and hierarchy.
///
/// Frontend: `const toc = await invoke<ApiResponse<TocEntry[]>>('generate_toc', { projectId });`
#[tauri::command]
pub async fn generate_toc(
    pool: State<'_, SqlitePool>,
    project_id: String,
) -> Result<ApiResponse<Vec<crate::services::TocEntry>>, String> {
    let pool_ref = pool.inner().clone();

    // Verify project exists
    let project_repo = crate::repositories::ProjectRepository::new(pool_ref.clone());
    ...
    // Parse manuscript to get AST
    let manuscript_root = match project.manuscript_root {
        Some(ref root) => root.clone(),
        None => {
            return Ok(ApiResponse::err(
                "PARSE_002: No manuscript_root configured for this project.",
            ));
        }
    };

    let ast = match crate::services::ParserService::parse_manuscript(&project_id, &manuscript_root).await {
        Ok(ast) => ast,
        Err(e) => return Ok(ApiResponse::err(format!("PARSE_001: {}", e))),
    };

    // Build TOC entries from parsed chapters (flat — sub-entries empty for now)
    let toc: Vec<crate::services::TocEntry> = ast
        .chapters
        .iter()
        .enumerate()
        .map(|(i, ch)| crate::services::TocEntry {
            title: ch.title.clone(),
            href: format!("chapter-{:02}.xhtml", i + 1),
            sub_entries: Vec::new(),
        })
        .collect();

    Ok(ApiResponse::ok(toc))
}
