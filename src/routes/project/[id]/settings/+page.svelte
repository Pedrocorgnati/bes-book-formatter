<script lang="ts">
  ...
  import { SETTINGS_TABS, type SettingsTab } from '$lib/constants/ui-tabs';
  ...
  const activeTab = $derived<SettingsTab>(
    ($page.url.searchParams.get('tab') as SettingsTab) === SETTINGS_TABS.PROGRESS
      ? SETTINGS_TABS.PROGRESS
      : SETTINGS_TABS.INTEGRATION
  );
...
<div data-testid="project-settings-page" class="settings-page">
  ...
  <nav data-testid="settings-tabs" class="settings-tabs" aria-label={t('a11y.projectNav')}>
    <button
      ...
      class="settings-tab"
      class:settings-tab--active={activeTab === SETTINGS_TABS.INTEGRATION}
      aria-selected={activeTab === SETTINGS_TABS.INTEGRATION}
      role="tab"
      onclick={() => switchTab(SETTINGS_TABS.INTEGRATION)}
    >
    ...
    <button
      ...
      class="settings-tab"
      class:settings-tab--active={activeTab === SETTINGS_TABS.PROGRESS}
      aria-selected={activeTab === SETTINGS_TABS.PROGRESS}
      role="tab"
      onclick={() => switchTab(SETTINGS_TABS.PROGRESS)}
    >
  ...
  <section data-testid="settings-content" class="settings-content">
    {#if activeTab === SETTINGS_TABS.INTEGRATION}
    ...
    {:else if activeTab === SETTINGS_TABS.PROGRESS}
