<script lang="ts">
  ...
  $effect(() => {
    ...
    if (projects.length === 0 && !error) {
      try {
        const firstLaunch = localStorage.getItem(STORAGE_KEYS.FIRST_LAUNCH);
        if (!firstLaunch) {
          localStorage.setItem(STORAGE_KEYS.FIRST_LAUNCH, 'done');
          goto(ROUTES.IMPORT);
        }
      } catch {
    ...
    <div data-testid="dashboard-projects" class="dashboard__projects">
      {#each projects as project (project.id)}
        <a
          ...
          href={PROJECT_ROUTES.ROOT(project.id)}
